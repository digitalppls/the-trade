...
<template>
  ...
  <div class="px-5 min-h-screen flex justify-content-center align-items-center">
    <div class="border-1 surface-border surface-card border-round py-7 px-4 md:px-7 z-1">
      <div class="mb-4">
        <div class="text-900 text-xl font-bold mb-2">{{ $t('auth.register') }}</div>
        <span class="text-600 font-medium">{{ $t('auth.registerSubText') }}</span>
      </div>
      <div class="flex flex-column">
        ...
        <div class="mb-4 flex flex-wrap">
          <Checkbox name="checkbox" v-model="confirmed" class="mr-2" binary/>
          <label htmlFor="checkbox" class="text-900 font-medium mr-2">
            {{ $t('auth.iRead') }}
          </label>
          <a class="text-600 cursor-pointer hover:text-primary cursor-pointer">
            {{ $t('auth.termsAndCondition') }}
          </a>
        </div>
        <Button :label="$t('auth.register')" class="w-full mb-4"/>
        <span class="font-medium text-600"> {{ $t('auth.alreadyAccount') }} <a
          class="font-semibold cursor-pointer text-900 hover:text-primary transition-colors transition-duration-300"
          @click="navigateTo('login')">{{ $t('auth.login') }}</a> </span>
      </div>
    </div>
  </div>
  <AppConfig simple />
