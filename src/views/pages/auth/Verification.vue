...
export default defineComponent({
  setup () {
    ...
    return {
      layoutConfig,
      router,
      value1: ref<string | null>(null),
      value2: ref<string | null>(null),
      value3: ref<string | null>(null),
      value4: ref<string | null>(null),
      regexNum: /^\d+$/
    }
  },
  computed: {
...
<template>
  ...
  <div class="px-5 min-h-screen flex justify-content-center align-items-center">
    <div class="border-1 surface-border surface-card border-round py-7 px-4 md:px-7 z-1">
      <div class="mb-4">
        <div class="text-900 text-xl font-bold mb-2">{{ $t('auth.verifyTitle') }}</div>
        <span class="text-600 font-medium">{{ $t('auth.verifySubtitle') }}:</span>
