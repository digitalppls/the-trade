...
<template>
  ...
  <div class="px-5 min-h-screen flex justify-content-center align-items-center">
    <div class="border-1 surface-border surface-card border-round py-7 px-4 md:px-7 z-1">
      <div class="mb-4">
        <div class="text-900 text-xl font-bold mb-2">{{ $t('auth.loginTitle') }}</div>
        <span class="text-600 font-medium">{{ $t('auth.inputData') }}</span>
      </div>
      <div class="flex flex-column">
        ...
        <div class="mb-4 flex flex-wrap gap-3">
          <div>
            ...
            <label htmlFor="checkbox" class="text-900 font-medium mr-8"> {{ $t('auth.rememberMe') }} </label>
          </div>
          <a
